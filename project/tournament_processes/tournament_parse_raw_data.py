...
def TournamentParseRawData(pdga_number, raw_data, file_location):
    soup = BeautifulSoup(raw_data, "html.parser")
    logging.info("Starting TournamentParseRawData")
    event = {}
    #Basic info
    ...
    if "/global-event/results/" in raw_data:
        ...
        special_event = False
        logging.info('Not a special event')
    ...
    if special_event == False:
        ...
        try:
            ...
            event['event_title'] = "Unknown tournament"
        try:
        ...
        try:
            event['event_total_players'] = soup.find_all("td", class_="players")[0].text
        except:
            try:
                event['event_total_players'] = soup.find_all(class_="players")[1].text
            except:
                event['event_total_players'] = 0
