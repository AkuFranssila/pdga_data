...
def CrawlPlayers(first_id, last_id, crawl_all):
    ...
    for i in range(first_id, last_id):
        ...
        player_data = {}
        player_pdga_number = i
        player_pdga_page_link = 'https://www.pdga.com/player/' + str(i)
        try:
            player_name = soup.find('h1').text.split(' #')[0].strip()
            #ID is actice = 1
            player_id_active = True
        except:
            logging.error('Player ID %s in URL https://www.pdga.com/player/%s is not working', str(i), str(i))
            player_name = None
